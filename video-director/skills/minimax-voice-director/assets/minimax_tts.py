...
环境变量:
    ...
    MINIMAX_MUSIC_MODEL: 音乐模型 (可选，默认 music-2.6-free)
"""
...
def get_config() -> Dict[str, str]:
    ...
    return {
        ...
        "music_model": os.environ.get("MINIMAX_MUSIC_MODEL", "music-2.6-free")
    }
...
def _save_audio_payload(audio_payload: str, output_path: str) -> Dict[str, Any]:
    """保存 MiniMax 返回的音频内容。支持 hex 音频或临时 URL。"""
    ensure_output_dir(output_path)

    if audio_payload.startswith("http://") or audio_payload.startswith("https://"):
        response = requests.get(audio_payload, timeout=120)
        response.raise_for_status()
        audio_data = response.content
    else:
        audio_data = bytes.fromhex(audio_payload)
...
def text_to_audio(
    ...
    Args:
        ...
        voice_id: 声音 ID
        output_path: 输出文件路径
        model: 模型版本，默认读取 MINIMAX_TTS_MODEL，再退回 speech-2.8-hd
        speed: 语速 [0.5, 2.0]
        vol: 音量 [0.1, 10.0]
        pitch: 音调 [-12, 12]
        emotion: 可选的历史情感字段。Speech 2.8 下仅在实测证明有效时使用
        format: 输出格式 (mp3, wav, pcm, flac)
        sample_rate: 采样率
        bitrate: 比特率
        channel: 声道数，1 为单声道
        language_boost: 语言增强，默认 auto
        subtitle_enable: 是否请求 MiniMax 返回字幕元数据
        subtitle_type: sentence, word 或 word_streaming
        output_format: 非流式响应使用 hex 或 url
        timeout: HTTP 超时秒数
    ...
    """
    config = get_config()
    voice_id = voice_id or os.environ.get("MINIMAX_VOICE_ID") or "female-shaonv"
    model = model or os.environ.get("MINIMAX_TTS_MODEL") or "speech-2.8-hd"
    if speed is None:
    ...
    try:
        ...
        if "audio" in data:
            saved = _save_audio_payload(data["audio"], output_path)
            response_metadata = {
                key: value for key, value in data.items() if key != "audio"
            }
...
def generate_music(
    ...
    """
    调用 MiniMax Music Generation API 生成音乐。

    Args:
        prompt: 音乐描述。纯音乐和 cover 模型必填。
        output_path: 输出文件路径。output_format 为 hex 或返回 URL 时都可保存。
        model: music-2.6, music-cover, music-2.6-free, music-cover-free。
        lyrics: 歌词。非纯音乐的 music-2.6/music-2.6-free 通常需要。
        lyrics_optimizer: 是否让模型根据 prompt 自动生成歌词。
        is_instrumental: 是否生成纯音乐。用于背景音乐时应设为 True。
        output_format: hex 或 url。url 有效期通常为 24 小时。
        sample_rate: 输出采样率。
        bitrate: 输出码率。
        format: 输出格式，mp3/wav/pcm。
        aigc_watermark: 是否添加 AIGC 水印。
        audio_url/audio_base64/cover_feature_id: cover 模型参考音频输入。
        stream: 是否流式返回。保存文件时通常保持 False。
        timeout: 请求超时时间，音乐生成可能需要更久。

    Returns:
        dict: success, file_path/audio_url, trace_id, extra_info 或 error。
    """
    config = get_config()
    ...
    if model in ["music-2.6", "music-2.6-free"]:
        if not is_instrumental and not lyrics and not lyrics_optimizer:
            return {
                "success": False,
                "error": "非纯音乐生成需要提供 lyrics，或设置 lyrics_optimizer=True"
            }
    elif model in ["music-cover", "music-cover-free"]:
        supplied_refs = [bool(audio_url), bool(audio_base64), bool(cover_feature_id)]
        if sum(supplied_refs) != 1:
            return {
                "success": False,
                "error": "cover 模型必须且只能提供 audio_url、audio_base64 或 cover_feature_id 之一"
            }

    output_path_expanded = None
    if output_path:
        output_path_expanded = os.path.expanduser(output_path)
    elif output_format == "hex":
        ensure_output_dir()
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path_expanded = os.path.join(
            config["output_dir"],
            f"music_{timestamp}.{format}"
        )

    url = f"{config['api_host']}/v1/music_generation"
    headers = {
        "Authorization": f"Bearer {config['api_key']}",
        "Content-Type": "application/json"
    }
    ...
    try:
        ...
        if base_resp and base_resp.get("status_code", 0) != 0:
            return {
                "success": False,
                "error": base_resp.get("status_msg", "音乐生成失败"),
                "error_code": base_resp.get("status_code"),
                "trace_id": result.get("trace_id")
            }

        audio_payload = result.get("data", {}).get("audio")
        if not audio_payload:
            return {
                "success": False,
                "error": "响应中没有 data.audio",
                "trace_id": result.get("trace_id"),
                "raw": result
            }
        ...
        if audio_payload.startswith("http://") or audio_payload.startswith("https://"):
            output["audio_url"] = audio_payload

        if output_path_expanded:
            saved = _save_audio_payload(audio_payload, output_path_expanded)
            output["file_path"] = saved["file_path"]
            output["size"] = saved["size"]
...
def generate_instrumental_music(
    ...
    **kwargs
    ...
    Args:
        prompt: 音乐描述。
        output_path: 输出文件路径。
        model: 默认读取 MINIMAX_MUSIC_MODEL，未设置时使用 music-2.6-free。
        **kwargs: 透传给 generate_music，例如 sample_rate、bitrate、format。
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("  - generate_music(prompt, output_path, ...)")
        print("  - generate_instrumental_music(prompt, output_path, ...)")
