...
- 时间精度统一到 0.1s
- HyperFrames 路线的「组件」ID 必须来自 `components-catalog.md` 登记的真实 ID，不许自创
- 由 `video-director` 选择 Remotion 时，组件使用 `remotion:[可读的组件角色]`，并在 `render-plan.md` 映射到真实实现
...
- **组件**：HyperFrames 路线必须是 components-catalog.md 登记过的 ID；找不到 → `broll-abstract.placeholder` 占位 + 在 § 9 登记。Remotion 路线使用 `remotion:[可读的组件角色]`，并在 `render-plan.md` 映射
...
- [ ] 每个 Scene 时间精度 0.1s；HyperFrames 组件 ID 真实，或 Remotion 组件角色已在 render-plan.md 映射
