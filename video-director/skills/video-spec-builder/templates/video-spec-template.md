...
- 分层/景深：[要不要把画面拆成背景、主体、前景、文字等独立层；要的话写清每类层怎么动，不需要就写不需要]
...
[纸片风、景深动画、卷轴、插画动起来等场景才填写。逐项列出背景、建筑/道具、人物、前景、文字、氛围层；每项写清来源、透明背景需求、画面位置和运动方式。没有就写「无分层素材需求」]
...
- 组件：[HyperFrames 路线填 components-catalog.md 的真实组件 ID；由 video-director 选择 Remotion 时填 `remotion:[可读的组件角色]`，并在 render-plan.md 映射到真实实现]
