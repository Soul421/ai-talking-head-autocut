...
name: baocut
description: >-
  Operate the established BaoCut CLI workflow when the user names BaoCut,
  requests a BaoCut project operation, or an upstream transcription/voice skill
  explicitly delegates to BaoCut. Do not trigger for generic video, music,
  ordinary code work inside the BaoCut repository, or another renderer.
metadata:
  version: "0.1.2"
  minAppVersion: "0.1.7"
---
...
本 Skill 的 `bin/baocut` 保留旧 CLI 0.2.0 入口。声音工作室已单独适配并默认使用 `~/.local/share/voice-studio/baocut/1.1.4/bcut`；新接口、转写和导出由声音工作室适配层负责，不能把本文旧命令传给新版二进制。

新版已用真实音频通过本地转写、专名替换、SRT/VTT 严格检查及 Markdown 导出，旧版应用未覆盖。声音工作室可通过 `VOICE_STUDIO_BAOCUT` 显式选择旧入口；不得在一次失败中自动切换重跑。旧 CLI 当前报告本地 ASR 模型未安装，旧接口回归测试通过但旧转写实跑未通过；恢复旧环境前不要宣称旧版可立即回退生产。
